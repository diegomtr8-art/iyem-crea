{{-- F-PR-OCC-02 R00 — Solicitud de Crédito Persona Física (Emprendedores / Sustentable) --}}
@php
    $codigo = 'F-PR-OCC-02 R00';
    $titulo = 'Solicitud de Crédito — Persona Física';
...
<div class="seccion">
    ...
    <div class="grid-2">
        <div class="col">
            ...
            <div class="campo"><div class="campo-label">RFC (con homoclave)</div><div class="campo-valor" style="font-family:monospace">{{ $val($solicitud->rfc) }}</div></div>
        ...
        <div class="col">
            ...
            <div class="campo"><div class="campo-label">Discapacidad</div>
                ...
                <div style="font-size:8px;margin-top:2px">Especifique: {{ $ext['discapacidad_tipo'] }}</div>
                @endif
            </div>
            <div class="campo"><div class="campo-label">Empleado administración pública</div>
                <div class="check-row">
                    <div class="check-item"><span class="check-box {{ ($ext['empleado_gobierno'] ?? false) ? 'check-si' : '' }}">{{ ($ext['empleado_gobierno'] ?? false) ? '✓' : '' }}</span> Sí</div>
                    <div class="check-item"><span class="check-box {{ !($ext['empleado_gobierno'] ?? false) ? 'check-si' : '' }}">{{ !($ext['empleado_gobierno'] ?? false) ? '✓' : '' }}</span> No</div>
                </div>
                @if($ext['empleado_gobierno'] ?? false)
                <div style="font-size:8px;margin-top:2px">Dependencia: {{ $ext['dependencia_gobierno'] ?? '' }} — Puesto: {{ $ext['puesto_gobierno'] ?? '' }}</div>
                @endif
            </div>
            @if($ext['nombre_conyuge'] ?? '')
            <div class="campo"><div class="campo-label">Cónyuge</div><div class="campo-valor">{{ $ext['nombre_conyuge'] }} — CURP: {{ $ext['curp_conyuge'] ?? '' }}</div></div>
            @endif
        </div>
    </div>
</div>
...
<div class="seccion">
    <div class="seccion-titulo">II. Domicilio del Solicitante</div>
    <div class="campo"><div class="campo-label">Dirección</div><div class="campo-valor">{{ $val($solicitud->direccion) }}</div></div>
    <div class="grid-4" style="margin-top:4px">
        <div class="col"><div class="campo"><div class="campo-label">Colonia</div><div class="campo-valor">{{ $val($ext['colonia'] ?? '') }}</div></div></div>
        <div class="col"><div class="campo"><div class="campo-label">C.P.</div><div class="campo-valor">{{ $val($ext['cp'] ?? '') }}</div></div></div>
        <div class="col"><div class="campo"><div class="campo-label">Municipio</div><div class="campo-valor">{{ $val($solicitud->municipio) }}</div></div></div>
        <div class="col"><div class="campo"><div class="campo-label">Domicilio</div>
            <div class="check-row">
            ...
            @if(!($ext['domicilio_propio'] ?? true))<div style="font-size:8px">Renta: ${{ number_format((float)($ext['renta_mensual'] ?? 0), 2) }}/mes</div>@endif
        </div></div>
    </div>
...
<div class="seccion">
    <div class="seccion-titulo">III. Referencia Personal (No familiar)</div>
    <div class="grid-3">
        ...
        <div class="col"><div class="campo"><div class="campo-label">C.P.</div><div class="campo-valor">{{ $val($ext['referencia_cp'] ?? '') }}</div></div></div>
    </div>
</div>

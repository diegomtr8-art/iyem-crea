{{-- F-PR-OCC-06 R00 — Declaración de Bienes Patrimoniales Aval (Artesanal) --}}
@php
    $codigo = 'F-PR-OCC-06 R00';
    $titulo = 'Declaración de Bienes Patrimoniales — Aval Artesanal';
...
<div class="seccion">
    ...
    <div class="grid-4" style="margin-top:4px">
        ...
        <div class="col"><div class="campo"><div class="campo-label">Domicilio</div>
            ...
            @if(!$av->domicilio_propio)<div style="font-size:8px">Renta: ${{ $n($av->renta_mensual) }}/mes</div>@endif
        </div></div>
    </div>
</div>

{{-- SITUACIÓN LABORAL DEL AVAL --}}
<div class="seccion">
    <div class="seccion-titulo">III. Situación Laboral</div>
    <div class="grid-2">
        <div class="col">
        ...
        <div class="col">
            ...
            <div class="campo"><div class="campo-label">Fecha inicio actividades</div><div class="campo-valor">{{ $av->fecha_inicio_actividades?->format('d/m/Y') }}</div></div>
        </div>
    </div>
</div>
...
<div class="seccion">
    ...
    <table>
        <thead><tr><th>Institución / Acreedor</th><th class="text-right">Monto ($)</th><th>Vencimiento</th><th>Garantía</th></tr></thead>
        <tbody>
            @forelse($hipot as $h)
            ...
            @endforelse
            @foreach($otras as $od)
            <tr><td>{{ $od['nombre'] ?? '' }}</td><td class="text-right">${{ $n($od['monto'] ?? 0) }}</td><td>{{ $od['vencimiento'] ?? '' }}</td><td>—</td></tr>
            @endforeach
            <tr style="background:#006847;color:#fff"><td class="bold">Total Deudas</td><td class="text-right bold">${{ $n($totalDeudas) }}</td><td colspan="2"></td></tr>
        </tbody>
    </table>
</div>
